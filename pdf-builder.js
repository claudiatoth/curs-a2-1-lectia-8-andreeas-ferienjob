// PDF BUILDER — Andreeas Ferienjob (A2.1 · Lecția 8)
(function () {
    ...
    function buildCast() {
        return `<div class="cast-banner">
            ...
            <div class="cast-grid">
                <div class="cast-card"><img src="images/andreea.png" alt="Andreea"><div class="name">Andreea 🇷🇴</div><div class="detail">Chelneriță în vacanță</div></div>
                <div class="cast-card"><img src="images/mihai.png" alt="Mihai"><div class="name">Mihai</div><div class="detail">Șeful · restaurantul</div></div>
    ...
    function buildTheory() {
        ...
        t = t.replace(/<div class="theory-box"\s+style="background:\s*#F5F0E8[^"]*"[^>]*>/g, '<div class="theory-box warn-box">');
        return `<h1 class="chapter">📘 1. Teorie — Andreeas Ferienjob: Wer? Wen? Wem? Was? + Dativobjekt</h1>` + t;
    }
    ...
    function buildExercises() {
        ...
        if (typeof ex1Data !== 'undefined') {
            html += fillPromptBlock('Übung 1 — Wer / Wen / Wem / Was?', 'Wer = cine (subiect) · Wen = pe cine (Akk) · Wem = cui (Dativ) · Was = ce (lucru).', ex1Data);
        }
        if (typeof ex2Data !== 'undefined') {
            html += choiceBlock('Übung 2 — Wen sau Wem?', 'Akkusativ (sehen/besuchen/treffen/fragen/anrufen → Wen) vs Dativ (gehören/helfen/danken/geben persoanei → Wem).', ex2Data);
        }
        if (typeof ex3Data !== 'undefined') {
            html += fillPromptBlock('Übung 3 — Articolul/pronumele la Dativ', 'der→dem · die→der · das→dem · plural die→den (+ -n: den Kindern, den Gästen).', ex3Data);
        }
        if (typeof ex4Data !== 'undefined') {
            html += fillPromptBlock('Übung 4 — Ordinea obiectelor (Dativ + Akkusativ)', 'Persoana (Dativ) ÎNAINTEA lucrului (Akkusativ): dem Gast das Bier.', ex4Data);
        }
        if (typeof ex5Data !== 'undefined') {
            html += translateBlock('Übung 5 — Traducere RO → DE', 'Atenție la Wer/Wen/Wem/Was și la ordinea Dativ + Akkusativ.', ex5Data);
        }
        return html;
    }
    ...
    function buildVerbs() {
        let html = `<h1 class="chapter new-section">🔁 4. Verbele lecției — Präsens · Präteritum · Perfekt</h1>
            <div class="andreea-note">
                <div class="andreea-note-content">
                    ...
                    <div>Verbele care „dau ceva cuiva" (geben, bringen, zeigen, schenken) au DOUĂ obiecte: persoana (Dativ) ÎNAINTEA lucrului (Akkusativ). Verbele helfen, danken, gratulieren, gehören cer DOAR Dativ — chiar dacă în română par cu acuzativ. Verbele TARI sunt verificate pe PONS.</div>
                </div>
            </div>`;
        if (typeof verbsData !== 'undefined') {
            verbsData.forEach((v, idx) => {
                ...
                else if (v.typ && v.typ.indexOf('mixt') === 0) { badgeClass = 'strong'; badgeLabel = 'MIXT (neregulat)'; }
